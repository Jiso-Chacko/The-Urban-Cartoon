...
router.get('/login', function(req, res, next) {
  if(req.session.adminLogin){
    ...
    res.render('admin/login',{"loginErr" : req.session.adminLoginErr,"loginNull" : req.session.adminLoginNull});
...
router.get('/addProduct',verifyAdminLogin, async function(req, res, next) {
  ...
  res.render('admin/addProducts',{layout : 'admin/layout', category : category, smartPhoneBrands : smartPhoneBrands , laptopBrands : laptopBrands, smartphoneTrue : req.session.smartphoneValue, laptopTrue : req.session.laptopValue});
...
const imageUpload = upload.fields([{ name: 'fileinputimage1', maxCount: 1 }, { name: 'fileinputimage2', maxCount: 1 },{ name: 'fileinputimage3', maxCount: 1 }, { name: 'fileinputimage4', maxCount: 1 }]) // product image upload
...
router.get('/viewProducts', verifyAdminLogin,function(req, res, next) {
  adminProductHelper.getallProducts().then((products) => {
    ...
    res.render('admin/viewProduct',{layout : 'admin/layout', product : products ,  imageName : imageName})
  })
});
...
router.get('/addSlider',verifyAdminLogin, function(req, res, next) { 
  ...
  adminProductHelper.getAllSlider().then((slider) => {
    ...
    // console.log(slider)
    res.render('admin/addSlider',{layout : 'admin/layout', slider : slider})
    })
  
});

const sliderImageUpload = upload.fields([{ name: 'fileinputimage1', maxCount: 1 }]) // slider image upload
...
router.get('/addBanner',verifyAdminLogin, function(req, res, next) { 
  ...
  adminProductHelper.getAllBanner().then((banner) => {
    ...
    // console.log(slider)
    res.render('admin/addBanner',{layout : 'admin/layout', banner : banner})
    })
  
});


const bannerImageUpload = upload.fields([{ name: 'fileinputimage1', maxCount: 1 }]) // banner image upload
